...
export const ARTICLES: Article[] = [
  ...
  },
  {
    ...
    updated: "2026-06-03",
    blocks: [
      { kind: "p", text: "This is the shortest path from nothing to a sovereign node answering on your own machine. Two commands stand up the runtime and bring StratosAgent online; you bring your own model key." },
      { kind: "callout", variant: "note", text: "The public installer endpoint opens at launch. Until then these commands describe the intended flow; join early access from the homepage to get a working endpoint." },
      { kind: "h2", id: "install-runtime", text: "1 · Install the runtime" },
      { kind: "tabs", tabs: [
        { label: "macOS / Linux", lang: "bash", code: "# install the Atmosphere runtime — no open ports\ncurl -fsSL https://get.efficientlabs.ai | sh" },
        { label: "Windows", lang: "powershell", code: "# install the Atmosphere runtime — no open ports\nirm https://get.efficientlabs.ai/win | iex" },
      ] },
      { kind: "h2", id: "bring-online", text: "2 · Bring StratosAgent online" },
      { kind: "code", lang: "bash", code: "stratos init    # generate local keys, seal the vault\nstratos up      # mesh into the Atmosphere, start channel adapters" },
      { kind: "p", text: "Keys are generated locally and never leave the machine. The node hole-punches into the mesh and becomes reachable across your configured channels — still fully sovereign." },
  ...
  },
  {
    ...
    updated: "2026-06-03",
    blocks: [
      { kind: "p", text: "A single installer brings up the Atmosphere runtime — no firewall holes, no inbound ports, nothing to expose. StratosAgent then provisions on the machine you already own." },
      { kind: "h2", id: "one-command", text: "One command" },
      { kind: "tabs", tabs: [
        { label: "macOS / Linux", lang: "bash", code: "curl -fsSL https://get.efficientlabs.ai | sh" },
        { label: "Windows", lang: "powershell", code: "irm https://get.efficientlabs.ai/win | iex" },
      ] },
      { kind: "h2", id: "your-hardware", text: "Your hardware" },
      { kind: "code", lang: "bash", code: "stratos init && stratos up" },
      { kind: "p", text: "Keys are generated locally and never leave the device. The node then hole-punches into the global mesh and becomes reachable across every configured channel." },
