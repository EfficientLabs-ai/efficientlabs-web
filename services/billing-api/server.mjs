...
const pool = new Pool(process.env.DATABASE_URL
  ...
  : {
      ...
      database: process.env.PGDATABASE || "efficientlabs",
      user: process.env.PGUSER || process.env.USER || "neo",
      max: Number(process.env.POSTGRES_POOL_MAX || 5),
      allowExitOnIdle: true,
      idleTimeoutMillis: 30_000,
      connectionTimeoutMillis: 5_000,
...
function planForPriceId(priceId) {
  ...
  const entries = [
    [process.env.STRIPE_PRICE_EXOS_PRO_MONTHLY, "exos_pro"],
    [process.env.STRIPE_PRICE_EXOS_PRO_ANNUAL, "exos_pro"],
    [process.env.STRIPE_PRICE_APEX_MONTHLY, "apex"],
    [process.env.STRIPE_PRICE_APEX_ANNUAL, "apex"],
    [process.env.STRIPE_PRICE_TEAMS_MONTHLY, "teams"],
    [process.env.STRIPE_PRICE_TEAMS_ANNUAL, "teams"],
  ];
...
async function handleHealth(_req, res) {
  try {
    ...
    return json(res, 200, {
      ...
      database: "ok",
      stripe: stripe && stripeWebhookSecret ? "configured" : "unconfigured",
    });
  } catch {
    return json(res, 503, {
      status: "unhealthy",
